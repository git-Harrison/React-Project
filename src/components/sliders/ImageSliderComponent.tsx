...
const ImageSliderComponent: React.FC = () => {
    ...
    const animateSlide = (index: number) => {
        ...
        if (slide) {
            const title = slide.querySelector('.slider-desc__title');
            const desc = slide.querySelector('.slider-desc__text');
            const btn = slide.querySelector('.link-btn');

            gsap.set([title, desc, btn], { y: 100, opacity: 0 }); // 초기 위치 설정
            gsap.to(title, { y: 0, opacity: 1, duration: 1, delay: 0.1 });
            gsap.to(desc, { y: 0, opacity: 1, duration: 1, delay: 0.3 });
            gsap.to(btn, { y: 0, opacity: 1, duration: 1, delay: 0.5 });
        }
    };
